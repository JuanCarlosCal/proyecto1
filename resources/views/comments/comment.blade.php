    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="text-center rounded-md p-2">
                <span class="text-black text-xl font-semibold">{{ session('message') }}</span>
            </div>
            @endif
            <div class="overflow-hidden shadow-sm sm:rounded-lg mb-4">
                <div class="p-6 text-gray-900 dark:text-gray-100s space-x-8">
                    <a href="{{ route('create') }}" class="px-4 py-4 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-sm text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700">{{ __('Agregar') }}</a>
                    <a href="#" class="px-4 py-4 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-sm text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700">{{ __('Los mejores comentarios') }}</a>
                </div>
            </div>
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                @forelse($comments as $comment)
                    <div class="p-6 flex space-x-2">
                    <svg fill="#0073b6" width="24px" height="24px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" transform="rotate(0)">
                    ...
                    <div class="flex-1 pl-3">
                        <div class="flex justify-between items-center">
                            <div>
                                ...
                                <small class="ml-2 text-sm text-gray-600 dark:text-gray-100">{{ $comment->created_at->format('d/m/Y') }}</small>
                            ...
                            @auth
                                <x-dropdown>
                                    ...
                                    <x-slot name="content">
                                        <x-dropdown-link :href="route('show', $comment)">
                                        {{ __('Ver') }}
                                        </x-dropdown-link>
                        ...
                        <p class="mt-3 text-lg text-gray-900 dark:text-gray-100">{{ $comment->title }}</p>
                ...
                @empty
                    <h2 class="text-xl text-white p-4">No hay comentarios almacenadas!</h2>
                @endforelse
            </div>
        </div>
    </div>
